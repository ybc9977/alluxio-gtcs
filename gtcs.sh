...
python3 ~/Downloads/flintrock-master/standalone.py copy-file -y gtcs /Users/ybc/Downloads/alluxio-gtcs.zip /home/ec2-user/
#mvn
...
do
    ...
    ssh -i ~/.ssh/gtcs.pem ${line} " ~/alluxio-gtcs/bin/alluxio-start.sh worker SudoMount; ~/alluxio-gtcs/bin/alluxio-start.sh client"
done < ~/flintrock.txt

read -r line < ~/flintrock.txt
ssh -i ~/.ssh/gtcs.pem ${line} "cd alluxio-gtcs;sh master.sh"
...
do
    ...
    scp -i ~/.ssh/gtcs.pem -r ${line}:~/alluxio-gtcs/$i.zip ~/Downloads/
    $i=$i+1
done < ~/flintrock.txt
