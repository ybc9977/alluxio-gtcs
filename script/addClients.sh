...
do
    ...
    ssh -o StrictHostKeyChecking=no -i $flintrockPemPath ${line} "sudo yum update -y;sudo yum install java-1.8.0-openjdk* -y;" < /dev/null
    ...
    ssh -o StrictHostKeyChecking=no -i $flintrockPemPath ${line} "cp ~/alluxio-gtcs/conf/alluxio-site.properties.template ~/alluxio-gtcs/conf/alluxio-site.properties" < /dev/null
    ...
    ssh -o StrictHostKeyChecking=no -i $flintrockPemPath ${line} "echo 'alluxio.worker.memory.size=8GB' >> ~/alluxio-gtcs/conf/alluxio-site.properties" < /dev/null
