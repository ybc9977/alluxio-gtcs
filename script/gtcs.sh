...
#set up
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "sudo yum update -y"
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "sudo yum install java-1.8.0-openjdk* -y"
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs 'export JAVA_HOME="/usr/lib/jvm/java-1.8.0-openjdk"'

python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "sudo wget http://repos.fedorapeople.org/repos/dchen/apache-maven/epel-apache-maven.repo -O /etc/yum.repos.d/epel-apache-maven.repo"
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs 'sudo sed -i s/\$releasever/7/g /etc/yum.repos.d/epel-apache-maven.repo'
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "sudo yum -y install apache-maven"

python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py describe gtcs

python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "sudo yum -y install git"
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "git clone https://github.com/ybc9977/alluxio-gtcs.git"


python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "pip2 install numpy==1.14.4"
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "pip2 install cvxpy==0.4.9"
...
python3 $(cd `dirname $0`; cd ..; pwd)/flintrock/standalone.py run-command gtcs "cd alluxio-gtcs;mvn clean install -DskipTests=true -Dlicense.skip=true -Dcheckstyle.skip -Dmaven.javadoc.skip=true"
