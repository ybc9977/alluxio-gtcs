...
public final class GameSystemMaster {
    ...
    /** Store user's utilities calculated */
    ...
    /** A user-file ArrayList, storing the files (ordered) that user has generated preference for */
    ...
    /**
     * A function to read ratio and factor information from related files generated by OpuS/FairRide
     * @param f the Data input stream of files needed to read
     * @param fileSystemMaster fileSystemMaster needed in cacheIt method calling
     * @throws IOException
     */
    ...
    private static void readAndCache(DataInputStream f, FileSystemMaster fileSystemMaster) throws IOException {
        ...
        // Use cacheList to remember files and their cached ratio
        ...
        //  Let clients cache the files they required
        for (String userId : cacheMap.keySet()){
            for (String file : cacheMap.get(userId)){
                clientList.get(userId).cacheIt(file,cacheList,fileSystemMaster);
            }
        }

//        //  Randomly select a client to execute cacheIt process, deprecated
...
//            LOG.info("Iter num: " + poll_iter + " Time cost : " + (System.currentTimeMillis()-iter_start));
    ...
    private void HitRatio() throws AlluxioStatusException {
        ...
        for (String user : clientList.keySet()){
            ...
            accessList=clientList.get(user).simulated_access(userPref.get(user));
            double h=0,acc=0;
